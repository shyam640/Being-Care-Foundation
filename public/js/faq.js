(function(){
	// FAQ Template
	// Creating class and constructor method (ES6 2015)
  class FaqTemplate {
		constructor(element) {
...
//   };
  ...
  function initFaqEvents(faqs) {
  	// click on a faq category
		faqs.faqsCategoriesContainer.addEventListener('click', function(event){
			var category = event.target.closest('.cd-faq__category');        // selecting from category Basics,Mobile,Account,Payment,Privacy,Delivery
			if(!category) return;
			var mq = getMq(faqs),
  ...
  function getMq(faqs) {
		return window.getComputedStyle(faqs.element, '::before').getPropertyValue('content').replace(/'|"/g, "");         // Regular expression , where g is for global this line means replacing ' or " with "" (space)
  };
